...
type EndSequence = { result: "won" | "lost"; started: number; until: number; reason: string };
type Runtime = { running: boolean; startTime: number; lastTime: number; elapsed: number; score: number; highestLevel: number; nextId: number; nextShakeAt: number; lastSpawnAt: number; lastHudAt: number; player: Player; toppings: Topping[]; target: { x: number; y: number; active: boolean }; pointer: { x: number; y: number; active: boolean }; joystick: { x: number; y: number; power: number; active: boolean }; keys: Set<string>; event: ActiveEvent; camera: { x: number; y: number }; shakeAngle: number; shakePower: number; endSequence?: EndSequence };
...
function updateCamera(runtime: Runtime) { const viewW = WIDTH / VIEW_SCALE, viewH = HEIGHT / VIEW_SCALE, followX = runtime.pointer.active ? runtime.player.x - runtime.pointer.x : runtime.player.x - viewW / 2, followY = runtime.pointer.active ? runtime.player.y - runtime.pointer.y : runtime.player.y - viewH / 2; runtime.camera.x += (clamp(followX, 0, WORLD_WIDTH - viewW) - runtime.camera.x) * 0.18; runtime.camera.y += (clamp(followY, 0, WORLD_HEIGHT - viewH) - runtime.camera.y) * 0.18; }
...
function triggerStraw(runtime: Runtime, now: number) { const stabs = buildStrawStabs(runtime); runtime.event = { kind: "strawWarning", until: now + Math.max(560, 980 - strawDifficulty(runtime) * 260), started: now, stabs }; }
...
function currentStrawStab(runtime: Runtime, now: number) {
  ...
  const thrust = phaseInfo.phase < 0.34 ? phaseInfo.phase / 0.34 : phaseInfo.phase < 0.7 ? 1 : 1 - (phaseInfo.phase - 0.7) / 0.3;
  const eased = clamp(thrust, 0, 1), x = stab.fromX + (stab.x - stab.fromX) * eased, y = stab.fromY + (stab.y - stab.fromY) * eased;
  return { x, y, target: { x: stab.x, y: stab.y }, fromX: stab.fromX, fromY: stab.fromY, active: phaseInfo.phase > 0.2 && phaseInfo.phase < 0.76 && phaseInfo.strike < stabs.length, strike, phase: phaseInfo.phase };
}
...
function drawStrawDangerRect(ctx: CanvasRenderingContext2D, stab: StrawStab, active: boolean, alpha: number) {
  const dx = stab.x - stab.fromX, dy = stab.y - stab.fromY, length = Math.hypot(dx, dy) + 88, width = active ? 92 : 82;
  ctx.save();
  ctx.translate((stab.fromX + stab.x) / 2, (stab.fromY + stab.y) / 2);
  ctx.rotate(Math.atan2(dy, dx) - Math.PI / 2);
  ctx.fillStyle = active ? "rgba(255,52,88," + (0.16 * alpha) + ")" : "rgba(255,52,88," + (0.22 * alpha) + ")";
  ctx.strokeStyle = active ? "rgba(255,255,255," + (0.58 * alpha) + ")" : "rgba(255,52,88," + (0.78 * alpha) + ")";
  ctx.lineWidth = active ? 5 : 4;
  ctx.setLineDash(active ? [18, 14] : [14, 10]);
  ctx.beginPath();
  ctx.roundRect(-width / 2, -length / 2, width, length, 16);
  ctx.fill();
  ctx.stroke();
  ctx.setLineDash([]);
  ctx.restore();
...
function drawScene(ctx: CanvasRenderingContext2D, runtime: Runtime | null, status: Status, assets: AssetImageMap = {}) {
  ...
  if (runtime) {
    ...
    if (runtime.event.kind === "strawWarning" || runtime.event.kind === "strawActive") {
      const now = performance.now(); const activeStab = runtime.event.kind === "strawActive" ? currentStrawStab(runtime, now) : null; const stabs = runtime.event.stabs ?? buildStrawStabs(runtime); const marks = activeStab ? [stabs[activeStab.strike] ?? stabs[0]] : stabs;
    ...
    if (runtime.event.kind === "strawActive") {
      const tip = currentStrawStab(runtime, performance.now());
      const strawImage = assetImage(assets, "scene-straw");
      if (strawImage) drawAssetBetween(ctx, strawImage, { x: tip.fromX, y: tip.fromY }, tip, 84, 88);
...
function updateRuntime(runtime: Runtime, dt: number, now: number, finish: (result: "won" | "lost", reason: string) => void) {
  ...
  if (runtime.endSequence) { const won = runtime.endSequence.result === "won"; player.vx = 0; player.vy = 0; runtime.target.active = false; runtime.pointer.active = false; runtime.joystick = { x: 0, y: 0, power: 0, active: false }; runtime.shakeAngle = Math.sin((now - runtime.endSequence.started) / (won ? 120 : 55)) * (won ? 0.035 : 0.055) * (1 - clamp((now - runtime.endSequence.started) / Math.max(1, runtime.endSequence.until - runtime.endSequence.started), 0, 1)); runtime.shakePower = won ? 0.25 : 0.42; updateCamera(runtime); if (now >= runtime.endSequence.until) finish(runtime.endSequence.result, runtime.endSequence.reason); return; }
  if (runtime.event.kind === "idle") { if (runtime.elapsed >= runtime.nextShakeAt) triggerShake(runtime, now); else if (runtime.elapsed >= 9000 && Math.random() < dt * (0.12 + difficulty * 0.43 + stage * 0.055)) triggerStraw(runtime, now); }
  else if (runtime.event.kind === "shakeWarning" && now >= runtime.event.until) { runtime.event = { ...runtime.event, kind: "shaking", until: now + 1850 + difficulty * 650 + stage * 120, started: now }; runtime.shakePower = 1; }
  else if (runtime.event.kind === "shaking" && now >= runtime.event.until) { runtime.score += 100; runtime.event = { kind: "idle", until: 0, started: now }; runtime.nextShakeAt = runtime.elapsed + rand(Math.max(4200, (11000 - difficulty * 4600 - stage * 360) / 1.3), Math.max(6200, (19000 - difficulty * 6500 - stage * 520) / 1.3)); }
  ...
  const speedPenalty = (levelInfo(player.level).scale - 1) * 10, maxSpeed = 220 - speedPenalty, controlVx = dirX * maxSpeed * inputPower, controlVy = dirY * maxSpeed * inputPower; player.vx = runtime.event.kind === "shaking" ? controlVx * 0.3 : controlVx; player.vy = runtime.event.kind === "shaking" ? controlVy * 0.3 : controlVy;
  if (runtime.event.kind === "shaking") { const elapsed = now - runtime.event.started, direction = runtime.event.swing ?? 1, pulse = Math.sin(elapsed / 42), swing = direction * pulse; runtime.shakeAngle = swing * (0.19 + difficulty * 0.04); runtime.shakePower = Math.max(0, 1 - elapsed / Math.max(2100, runtime.event.until - runtime.event.started)); const shakeStrength = 660 + difficulty * 520 + stage * 70, lateral = Math.cos(elapsed / 42) * direction; for (const item of [player, ...runtime.toppings]) { const dx = item.x - CUP_CENTER.x, dy = item.y - CUP_CENTER.y, len = Math.max(80, Math.hypot(dx, dy)); const tangentX = -dy / len, tangentY = dx / len, bottomBias = 0.65 + cupT(item.y) * 0.9, shakeWeight = item === player ? 0.7 : 1; item.vx += (tangentX * shakeStrength * swing + lateral * shakeStrength * 0.95) * bottomBias * dt * shakeWeight; item.vy += (tangentY * shakeStrength * swing + Math.sin(elapsed / 31) * (190 + stage * 22)) * bottomBias * dt * shakeWeight; } } else { runtime.shakeAngle *= 0.82; runtime.shakePower *= 0.84; }
  if (runtime.event.kind === "strawActive") { const tip = currentStrawStab(runtime, now), c = tip.target, dangerRadius = 43 + difficulty * 22 + stage * 5, pullRadius = 220 + difficulty * 76 + stage * 18, pull = 300 + difficulty * 245 + stage * 62, dPlayer = dist(player, tip); if (tip.active && dPlayer < dangerRadius + player.radius * 0.55) { runtime.endSequence = { result: "lost", started: now, until: now + 1000, reason: "\u4f60\u88ab\u5438\u7ba1\u6233\u5230\u4e86" }; runtime.event = { kind: "idle", until: 0, started: now }; return; } runtime.toppings = runtime.toppings.filter((item) => { const d = dist(item, tip); if (tip.active && d < dangerRadius + item.radius * 0.3) return false; if (d < pullRadius) { const power = (1 - d / pullRadius) * pull * 0.95; item.vx += (c.x - item.x) / Math.max(1, d) * power * dt; item.vy += (c.y - item.y) / Math.max(1, d) * power * dt; } return true; }); }
...
export function MilkTeaGame() {
  const canvasRef = useRef<HTMLCanvasElement | null>(null); const runtimeRef = useRef<Runtime | null>(null); const assetImagesRef = useRef<AssetImageMap>({}); const joystickKnobRef = useRef<HTMLSpanElement | null>(null); const [assetRevision, setAssetRevision] = useState(0); const [status, setStatus] = useState<Status>("menu"); const [hud, setHud] = useState<Hud>({ level: 1, progress: 1, score: 0, elapsed: 0, message: TXT.defaultHint }); const [finalStats, setFinalStats] = useState<FinalStats | null>(null); const [leaderboard, setLeaderboard] = useState<ScoreRecord[]>([]); const [playerName, setPlayerName] = useState(TXT.defaultName); const [saved, setSaved] = useState(false);
  const setTargetFromScreenPoint = (runtime: Runtime, screenX: number, screenY: number) => { let x = screenX + runtime.camera.x, y = screenY + runtime.camera.y; if (Math.abs(runtime.shakeAngle) > 0.001) { const dx = x - CUP_CENTER.x, dy = y - CUP_CENTER.y, c = Math.cos(-runtime.shakeAngle), s = Math.sin(-runtime.shakeAngle); x = CUP_CENTER.x + dx * c - dy * s; y = CUP_CENTER.y + dx * s + dy * c; } runtime.target = { x, y, active: true }; };
  const refreshPointerTarget = (runtime: Runtime) => { if (runtime.pointer.active && !runtime.joystick.active && runtime.running) setTargetFromScreenPoint(runtime, runtime.pointer.x, runtime.pointer.y); };
  const updateTargetFromClient = (clientX: number, clientY: number) => { const canvas = canvasRef.current, runtime = runtimeRef.current; if (!canvas || !runtime || !runtime.running) return; const rect = canvas.getBoundingClientRect(); const screenX = ((clientX - rect.left) / rect.width) * WIDTH / VIEW_SCALE, screenY = ((clientY - rect.top) / rect.height) * HEIGHT / VIEW_SCALE; runtime.pointer = { x: screenX, y: screenY, active: true }; setTargetFromScreenPoint(runtime, screenX, screenY); };
  useEffect(() => {
    let disposed = false;
