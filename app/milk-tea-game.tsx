...
function addToppings(runtime: Runtime, count: number, forcedLevel?: number) { for (let i = 0; i < count; i += 1) { const level = forcedLevel ?? chooseSpawnLevel(runtime.player.level, runtime.elapsed), radius = radiusForLevel(level); let point = randomCupPoint(radius); for (let attempts = 0; attempts < 50; attempts += 1) { point = randomCupPoint(radius); if (dist(point, runtime.player) > runtime.player.radius + radius + 55) break; } runtime.toppings.push({ id: runtime.nextId, x: point.x, y: point.y, vx: rand(-68, 68), vy: rand(-68, 68), level, radius, spin: rand(0, Math.PI * 2) }); runtime.nextId += 1; } }
...
function strawPeriod(runtime: Runtime) { const stage = difficultyStage(runtime.elapsed, runtime.player.level); return Math.max(165, (660 - strawDifficulty(runtime) * 180 - stage * 18) / 2); }
...
function triggerStraw(runtime: Runtime, now: number) { const stabs = buildStrawStabs(runtime); runtime.event = { kind: "strawWarning", until: now + Math.max(215, (754 - strawDifficulty(runtime) * 260) / 2), started: now, stabs }; }
