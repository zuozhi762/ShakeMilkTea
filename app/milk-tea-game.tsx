...
type Celebration = { started: number; until: number; reason: string };
type Runtime = { running: boolean; startTime: number; lastTime: number; elapsed: number; score: number; highestLevel: number; nextId: number; nextShakeAt: number; lastSpawnAt: number; lastHudAt: number; player: Player; toppings: Topping[]; target: { x: number; y: number; active: boolean }; joystick: { x: number; y: number; power: number; active: boolean }; keys: Set<string>; event: ActiveEvent; camera: { x: number; y: number }; shakeAngle: number; shakePower: number; celebration?: Celebration };
...
function strawPeriod(runtime: Runtime) { const stage = difficultyStage(runtime.elapsed, runtime.player.level); return Math.max(300, 650 - strawDifficulty(runtime) * 230 - stage * 28); }
...
function drawScene(ctx: CanvasRenderingContext2D, runtime: Runtime | null, status: Status, assets: AssetImageMap = {}) {
  ...
  if (runtime) {
    ...
    if (runtime.event.kind === "strawWarning" || runtime.event.kind === "strawActive") {
      const now = performance.now(); const activeStab = runtime.event.kind === "strawActive" ? currentStrawStab(runtime, now) : null; const marks = activeStab ? [activeStab.target] : (runtime.event.stabs ?? buildStrawStabs(runtime));
      const ringImage = assetImage(assets, "effect-suction-ring");
      marks.forEach((mark, index) => {
        const alpha = runtime.event.kind === "strawActive" && activeStab?.strike !== index ? 0.34 : runtime.event.kind === "strawActive" ? 0.74 : 0.6;
        if (ringImage) drawAssetCentered(ctx, ringImage, mark.x, mark.y, 142, 142, alpha);
        else { ctx.fillStyle = runtime.event.kind === "strawActive" ? "rgba(255,52,88,0.16)" : "rgba(255,52,88,0.24)"; ctx.beginPath(); ctx.arc(mark.x, mark.y, 245, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = "rgba(255,52,88,0.68)"; ctx.lineWidth = 4; ctx.setLineDash([12, 10]); ctx.beginPath(); ctx.arc(mark.x, mark.y, 62, 0, Math.PI * 2); ctx.stroke(); ctx.setLineDash([]); }
      });
      if (runtime.event.kind === "strawActive" && activeStab) { for (let ring = 0; ring < 4; ring += 1) { ctx.strokeStyle = "rgba(255,255,255," + (0.46 - ring * 0.07) + ")"; ctx.lineWidth = 4; ctx.beginPath(); ctx.arc(activeStab.x, activeStab.y, 36 + ring * 32 + Math.sin(runtime.elapsed / 58 + ring) * 8, 0.2 + ring, Math.PI * 1.66 + ring); ctx.stroke(); } ctx.fillStyle = "rgba(255,255,255,0.86)"; ctx.font = "700 22px sans-serif"; ctx.textAlign = "center"; ctx.fillText(String(activeStab.strike + 1) + "/3", activeStab.target.x, activeStab.target.y - 78); }
    }
    ...
    if (runtime.celebration) { const t = clamp((performance.now() - runtime.celebration.started) / 3000, 0, 1), pulse = 0.5 + Math.sin(t * Math.PI * 8) * 0.5; ctx.save(); ctx.textAlign = "center"; ctx.font = "800 34px sans-serif"; ctx.fillStyle = "rgba(255,245,206,0.96)"; ctx.strokeStyle = "rgba(121,63,21,0.72)"; ctx.lineWidth = 5; ctx.strokeText("\u5927\u67ff\u5b50\u5b8c\u6210\uff01", runtime.player.x, runtime.player.y - runtime.player.radius - 42); ctx.fillText("\u5927\u67ff\u5b50\u5b8c\u6210\uff01", runtime.player.x, runtime.player.y - runtime.player.radius - 42); for (let i = 0; i < 5; i += 1) { ctx.strokeStyle = "rgba(255,218,84," + (0.7 - i * 0.11) + ")"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(runtime.player.x, runtime.player.y, runtime.player.radius + 18 + i * 18 + pulse * 10, 0, Math.PI * 2); ctx.stroke(); } ctx.restore(); }
...
function updateRuntime(runtime: Runtime, dt: number, now: number, finish: (result: "won" | "lost", reason: string) => void) {
  ...
  if (runtime.celebration) { player.vx = 0; player.vy = 0; runtime.target.active = false; runtime.joystick = { x: 0, y: 0, power: 0, active: false }; runtime.shakeAngle = Math.sin((now - runtime.celebration.started) / 120) * 0.035; runtime.shakePower = 0.25; updateCamera(runtime); if (now >= runtime.celebration.until) finish("won", runtime.celebration.reason); return; }
  ...
  if (runtime.event.kind === "strawActive") { const tip = currentStrawStab(runtime, now), c = tip.target, dangerRadius = 43 + difficulty * 22 + stage * 5, pullRadius = 220 + difficulty * 76 + stage * 18, pull = 300 + difficulty * 245 + stage * 62, dPlayer = dist(player, tip); if (tip.active && dPlayer < dangerRadius + player.radius * 0.55) { finish("lost", "\u4f60\u88ab\u5438\u7ba1\u6233\u5230\u4e86"); return; } runtime.toppings = runtime.toppings.filter((item) => { const d = dist(item, tip); if (tip.active && d < dangerRadius + item.radius * 0.3) return false; if (d < pullRadius) { const power = (1 - d / pullRadius) * pull * 0.95; item.vx += (c.x - item.x) / Math.max(1, d) * power * dt; item.vy += (c.y - item.y) / Math.max(1, d) * power * dt; } return true; }); }
...
export function MilkTeaGame() {
  ...
  const stopJoystick = () => { if (runtimeRef.current) { runtimeRef.current.joystick = { x: 0, y: 0, power: 0, active: false }; runtimeRef.current.player.vx = 0; runtimeRef.current.player.vy = 0; } setJoystick({ active: false, knobX: 0, knobY: 0 }); };
