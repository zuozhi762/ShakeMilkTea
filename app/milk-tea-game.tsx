...
function chooseSpawnLevel(playerLevel: number, elapsed = 0) {
  const maxLevel = Math.min(9, playerLevel + 2); const pressure = Math.min(elapsed / 150000, 1);
  const weights = Array.from({ length: maxLevel }, (_, i) => { const level = i + 1; if (level === playerLevel + 2) return 0.7 + pressure * 3.1; if (level === playerLevel + 1) return 1.2 + pressure * 3.5; if (level === playerLevel) return 4.2; return Math.max(1.2, 2.9 - (playerLevel - level) * 0.35); });
...
function updateRuntime(runtime: Runtime, dt: number, now: number, finish: (result: "won" | "lost", reason: string) => void) {
  ...
  if (runtime.elapsed - runtime.lastSpawnAt > 2100) { runtime.lastSpawnAt = runtime.elapsed; const desired = 16 + Math.round(difficulty * 42) + runtime.player.level * 2; if (runtime.toppings.length < desired) addToppings(runtime, Math.min(7, desired - runtime.toppings.length)); }
