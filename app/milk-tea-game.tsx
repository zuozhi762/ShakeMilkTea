...
function drawScene(ctx: CanvasRenderingContext2D, runtime: Runtime | null, status: Status, assets: AssetImageMap = {}) {
  ...
  drawWorldBackground(ctx, runtime);
...
function updateRuntime(runtime: Runtime, dt: number, now: number, finish: (result: "won" | "lost", reason: string) => void) {
  ...
  for (const item of runtime.toppings) { const wander = 21 + item.level * 3.1; item.vx += Math.cos(now / 620 + item.spin) * wander * dt; item.vy += Math.sin(now / 760 + item.spin) * wander * dt; const levelSpeedRatio = 0.8 + (item.level - 1) / Math.max(1, LEVELS.length - 1) * 0.7, maxItemSpeed = runtime.event.kind === "shaking" ? maxSpeed * Math.min(1.5, levelSpeedRatio + 0.28) : maxSpeed * levelSpeedRatio, speed = Math.hypot(item.vx, item.vy); if (speed > maxItemSpeed) { item.vx = item.vx / speed * maxItemSpeed; item.vy = item.vy / speed * maxItemSpeed; } item.x += item.vx * dt; item.y += item.vy * dt; keepInCup(item); }
