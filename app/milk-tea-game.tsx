...
function updateRuntime(runtime: Runtime, dt: number, now: number, finish: (result: "won" | "lost", reason: string) => void) {
  ...
  if (dirX === 0 && dirY === 0 && runtime.pointer.active) { const screenX = player.x - runtime.camera.x, screenY = player.y - runtime.camera.y, dx = runtime.pointer.x - screenX, dy = runtime.pointer.y - screenY, len = Math.hypot(dx, dy); if (len > 5) { dirX = dx / len; dirY = dy / len; inputPower = clamp(len / 92, 0, 1); } } else if (dirX || dirY) { const len = Math.hypot(dirX, dirY); dirX /= len; dirY /= len; }
...
export function MilkTeaGame() {
  ...
  const updateTargetFromClient = (clientX: number, clientY: number) => { const canvas = canvasRef.current, runtime = runtimeRef.current; if (!canvas || !runtime || !runtime.running) return; const rect = canvas.getBoundingClientRect(); runtime.pointer = { x: ((clientX - rect.left) / rect.width) * WIDTH / VIEW_SCALE, y: ((clientY - rect.top) / rect.height) * HEIGHT / VIEW_SCALE, active: true }; runtime.target.active = false; };
  useEffect(() => {
    let disposed = false;
