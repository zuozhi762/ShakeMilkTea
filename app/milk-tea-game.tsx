...
function clampPointToCup(x: number, y: number, radius: number) { const cy = clamp(y, CUP.topY + radius, CUP.bottomY - radius); const b = cupBoundsAt(cy, radius); return { x: clamp(x, b.left, b.right), y: cy }; }
...
function chooseSpawnLevel(playerLevel: number) {
  ...
  const weights = Array.from({ length: maxLevel }, (_, i) => { const level = i + 1; if (level === playerLevel + 2) return 4.4; if (level === playerLevel + 1) return 5.4; if (level === playerLevel) return 3.9; return Math.max(0.9, 2.9 - (playerLevel - level) * 0.32); });
...
function createRuntime(): Runtime {
  ...
  updateCamera(runtime); addToppings(runtime, 22, 1); addToppings(runtime, 12, 2); addToppings(runtime, 8, 3); addToppings(runtime, 4, 4); return runtime;
}
function addToppings(runtime: Runtime, count: number, forcedLevel?: number) { for (let i = 0; i < count; i += 1) { const level = forcedLevel ?? chooseSpawnLevel(runtime.player.level), radius = radiusForLevel(level); let point = randomCupPoint(radius); for (let attempts = 0; attempts < 50; attempts += 1) { point = randomCupPoint(radius); if (dist(point, runtime.player) > runtime.player.radius + radius + 55) break; } runtime.toppings.push({ id: runtime.nextId, x: point.x, y: point.y, vx: rand(-52, 52), vy: rand(-52, 52), level, radius, spin: rand(0, Math.PI * 2) }); runtime.nextId += 1; } }
...
function drawScene(ctx: CanvasRenderingContext2D, runtime: Runtime | null, status: Status) {
  ...
  if (runtime) {
    ...
    if (runtime.event.kind === "strawWarning" || runtime.event.kind === "strawActive") { const c = runtime.event.kind === "strawActive" ? strawCenter(runtime, performance.now()) : { x: runtime.event.x ?? CUP.centerX, y: runtime.event.y ?? 320 }; ctx.fillStyle = runtime.event.kind === "strawActive" ? "rgba(255,52,88,0.16)" : "rgba(255,52,88,0.24)"; ctx.beginPath(); ctx.arc(c.x, c.y, 155, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = "rgba(255,52,88,0.56)"; ctx.lineWidth = 3; ctx.setLineDash([12, 10]); ctx.beginPath(); ctx.arc(c.x, c.y, 38, 0, Math.PI * 2); ctx.stroke(); ctx.setLineDash([]); if (runtime.event.kind === "strawActive") for (let ring = 0; ring < 5; ring += 1) { ctx.strokeStyle = "rgba(255,255,255," + (0.42 - ring * 0.06) + ")"; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(c.x, c.y, 30 + ring * 28 + Math.sin(runtime.elapsed / 100 + ring) * 7, 0.2 + ring, Math.PI * 1.62 + ring); ctx.stroke(); } }
...
function updateRuntime(runtime: Runtime, dt: number, now: number, finish: (result: "won" | "lost", reason: string) => void) {
  runtime.elapsed = now - runtime.startTime; const player = runtime.player; const difficulty = Math.min(runtime.elapsed / 150000, 1);
  ...
  for (let i = runtime.toppings.length - 1; i >= 0; i -= 1) { const item = runtime.toppings[i]; if (!isInsideCup(item.x, item.y, item.radius)) continue; if (dist(player, item) < player.radius + item.radius * 0.72) { if (item.level > player.level) { finish("lost", "\u649e\u4e0a\u4e86\u66f4\u5927\u7684" + levelInfo(item.level).name); return; } runtime.toppings.splice(i, 1); player.carried[item.level] += 1; if (item.level === player.level) { runtime.score += item.level * 20; player.progress += 1; if (player.progress >= 3) { player.level += 1; player.progress = 0; player.radius = radiusForLevel(player.level); runtime.highestLevel = Math.max(runtime.highestLevel, player.level); runtime.score += player.level * 100; player.vx *= 0.45; player.vy *= 0.45; if (player.level >= 10) { runtime.score += 3000; finish("won", "\u4f60\u5408\u6210\u4e86\u4f20\u8bf4\u4e2d\u7684\u67ff\u5b50"); return; } } } else runtime.score += item.level * 10; } }
  if (runtime.elapsed - runtime.lastSpawnAt > 1800) { runtime.lastSpawnAt = runtime.elapsed; const desired = 54 + Math.round(difficulty * 18) + runtime.player.level * 2; if (runtime.toppings.length < desired) addToppings(runtime, Math.min(10, desired - runtime.toppings.length)); }
  updateCamera(runtime);
}

export function MilkTeaGame() {
  const canvasRef = useRef<HTMLCanvasElement | null>(null); const runtimeRef = useRef<Runtime | null>(null); const [status, setStatus] = useState<Status>("menu"); const [hud, setHud] = useState<Hud>({ level: 1, progress: 0, score: 0, elapsed: 0, message: TXT.defaultHint }); const [finalStats, setFinalStats] = useState<FinalStats | null>(null); const [leaderboard, setLeaderboard] = useState<ScoreRecord[]>([]); const [playerName, setPlayerName] = useState(TXT.defaultName); const [saved, setSaved] = useState(false);
  ...
  const handlePointer = (event: React.PointerEvent<HTMLCanvasElement>) => { const canvas = canvasRef.current, runtime = runtimeRef.current; if (!canvas || !runtime) return; const rect = canvas.getBoundingClientRect(); const x = ((event.clientX - rect.left) / rect.width) * WIDTH / VIEW_SCALE + runtime.camera.x; const y = ((event.clientY - rect.top) / rect.height) * HEIGHT / VIEW_SCALE + runtime.camera.y; runtime.target = { ...clampPointToCup(x, y, runtime.player.radius), active: true }; };
