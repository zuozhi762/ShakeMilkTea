...
function updateRuntime(runtime: Runtime, dt: number, now: number, finish: (result: "won" | "lost", reason: string) => void) {
  ...
  const speedPenalty = (levelInfo(player.level).scale - 1) * 10, maxSpeed = 205 - speedPenalty;
  ...
  const controlVx = runtime.event.kind === "shaking" ? targetVx * 0.3 : targetVx, controlVy = runtime.event.kind === "shaking" ? targetVy * 0.3 : targetVy;
