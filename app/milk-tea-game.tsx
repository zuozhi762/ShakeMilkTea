...
type Runtime = { running: boolean; startTime: number; lastTime: number; elapsed: number; score: number; highestLevel: number; nextId: number; nextShakeAt: number; lastSpawnAt: number; lastHudAt: number; player: Player; toppings: Topping[]; target: { x: number; y: number; active: boolean }; pointer: { clientX: number; clientY: number; active: boolean }; joystick: { x: number; y: number; power: number; active: boolean }; keys: Set<string>; event: ActiveEvent; camera: { x: number; y: number }; shakeAngle: number; shakePower: number; endSequence?: EndSequence };
...
export function MilkTeaGame() {
  ...
  const refreshTargetFromClient = (runtime: Runtime, clientX: number, clientY: number) => { const canvas = canvasRef.current; if (!canvas) return; const rect = canvas.getBoundingClientRect(); let x = ((clientX - rect.left) / rect.width) * WIDTH / VIEW_SCALE + runtime.camera.x; let y = ((clientY - rect.top) / rect.height) * HEIGHT / VIEW_SCALE + runtime.camera.y; if (Math.abs(runtime.shakeAngle) > 0.001) { const dx = x - CUP_CENTER.x, dy = y - CUP_CENTER.y, c = Math.cos(-runtime.shakeAngle), s = Math.sin(-runtime.shakeAngle); x = CUP_CENTER.x + dx * c - dy * s; y = CUP_CENTER.y + dx * s + dy * c; } runtime.target = { x, y, active: true }; };
  const refreshPointerTarget = (runtime: Runtime) => { if (runtime.pointer.active && !runtime.joystick.active && runtime.running) refreshTargetFromClient(runtime, runtime.pointer.clientX, runtime.pointer.clientY); };
  const updateTargetFromClient = (clientX: number, clientY: number) => { const runtime = runtimeRef.current; if (!runtime || !runtime.running) return; runtime.pointer = { clientX, clientY, active: true }; refreshTargetFromClient(runtime, clientX, clientY); };
  useEffect(() => {
    let disposed = false;
